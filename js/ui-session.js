...
// Session flow: Declaration → Disciple Select → Strike → Metronome
...
(function () {
  ...
  const DISCIPLES = [
    { id: 'ap',      name: 'The Wellspring', course: 'A&P',          tarot: 'The Magician',     glyph: '✦', color: '#2AC878', unlock: null },
    { id: 'biochem', name: 'The Alchemist',  course: 'Biochemistry', tarot: 'Wheel of Fortune', glyph: '⬡', color: '#D4900A', unlock: null },
    { id: 'theo',    name: 'The Lantern',    course: 'Theory',       tarot: 'The Star',         glyph: '☆', color: '#6AAAD4', unlock: null },
    { id: 'de',      name: 'The Herald',     course: 'Deutsch',      tarot: 'The Sun',          glyph: '☀', color: '#E8C040', unlock: null },
    { id: 'ha',      name: 'The Mirror',     course: 'Health Assess.',tarot: 'Justice',          glyph: '⚖', color: '#4A9ACA', unlock: null },
    { id: 'fund',    name: 'The Wayfarer',   course: 'Fundamentals', tarot: 'The Fool',         glyph: '◈', color: '#9A70DA', unlock: null },
    { id: 'micro',   name: 'The Rot',        course: 'MicroPara',    tarot: 'The Devil',        glyph: '⸸', color: '#AA4A10', unlock: null },
    { id: 'nut',     name: 'The Garden',     course: 'Nutrition',    tarot: 'Temperance',       glyph: '⚘', color: '#50BA70', unlock: { year: 2, semester: 1 } },
    { id: 'pharm',   name: 'The Canon',      course: 'Pharmacology', tarot: 'The Emperor',      glyph: '⚕', color: '#CA4040', unlock: { year: 2, semester: 1 } },
    { id: 'matob',   name: 'The Cradle',     course: 'Maternal',     tarot: 'The Empress',      glyph: '♡', color: '#CA509A', unlock: { year: 2, semester: 1 } },
    { id: 'medsurg', name: 'The Warhorse',   course: 'Med-Surg',     tarot: 'Strength',         glyph: '⚔', color: '#5A78CA', unlock: { year: 3, semester: 1 } },
    { id: 'psych',   name: 'The Veil',       course: 'Psych',        tarot: 'High Priestess',   glyph: '◐', color: '#7A9ABA', unlock: { year: 3, semester: 2 } },
  ];
  ...
  // ─── STEP 2: STRIKE ───────────────────────────────────────────────────────
  // NOW: NO timer selection. Just show Disciple info and confirmation button.
  ...
  window.sessStrike = function () {
    ...
    const ok = startSession(_primaryId, _secondaryIds, false); // session.js, lightDay = false for now
    if (!ok) {
      if (btn) { btn.disabled = false; btn.innerHTML = '<span>✦</span> Strike'; }
      return;
    }
    metronomeStrike(); // metronome.js — uses defaults (52m rite, 17m reckoning)
    closeSessionOverlay();
  };
  ...
  function openSessionEndOverlay(reason) {
    ...
    if (vtEl) {
      ...
      vtEl.style.color  = earned > 0 ? 'var(--teal, #2AC878)' : 'var(--text-muted)';
    }
  ...
  window.endSessionEarly = function () {
    // Cost 60 vt from Sanctum if available
    const vt = getVt();
    if (vt < 60) {
      alert('Insufficient vt. You need 60 vt to end session early.');
      return;
    }
    spendVt(60);
    _showVtFlash('−60 vt');
    endSession('early');    // session.js
    metronomeReset();       // metronome.js
    resetSessionTotal();    // economy.js
    applySessionUI(false);
  ...
  function applySessionUI(active) {
    ...
    // hide Pomodoro FAB during session
    const pomoFab = document.getElementById('hub-pomo-mini');
    if (pomoFab) pomoFab.style.display = active ? 'none' : 'block';
  }
